...
<body>
    ...
    <script>
        ...
        pdf.addEventListener('click', async () => {
            <?php if ($_ENV['MGBROWSER_PLATFORM'] == 'linux') { ?>
                await mgbrowser.exportPDF('<?php echo dirname(__FILE__) . '/pdf/example.pdf'; ?>');
            <?php } else { ?>
                await mgbrowser.exportPDF('<?php echo str_replace('\\','\\\\', dirname(__FILE__)) . '\\\\pdf\\\\example.pdf'; ?>');
            <?php } ?>
            mgbrowser.newWindow('/pdf/example.pdf');
        });
    </script>
</body>
